...
export default function InterestCalculatorClient() {
  ...
  function validateNumbers(P, R, T) {
    if (!isFinite(P) || P <= 0) return "Enter a valid principal (> 0).";
    if (!isFinite(R) || R < 0) return "Enter a valid annual rate (≥ 0).";
    if (!isFinite(T) || T <= 0) return "Enter a valid time in years (> 0).";
    return "";
  }
  ...
  function calculate() {
    ...
    const validation = validateNumbers(P, R, T);
    if (validation) {
      setError(validation);
      return;
    }
  ...
  async function copyResult() {
    ...
    const txt =
      result.type === "simple"
        ...
        : `Compound interest (${result.frequency}×/yr): principal $${formatMoney(result.principal)}, interest $${formatMoney(result.interest)}, total $${formatMoney(result.total)}`;
    try {
    ...
    <div className="bg-gray-800/40 p-6 rounded-xl max-w-md mx-auto">
      <div className="space-y-4">
        ...
        <div className="mt-6 text-sm text-gray-400">
          Tip: enter fractional years for partial periods (e.g., 1.5 for 18 months). Monthly compounding yields slightly higher totals than yearly compounding.
        </div>
      </div>
    </div>
  );
